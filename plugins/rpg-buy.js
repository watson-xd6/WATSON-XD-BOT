...
${Object.keys(listItems).map((v) => {
        ...
        return `➠ 1 ${rpg.emoticon(v)} ${capitalize(v)} ﹫ ${listItems[v][paymentMethod]} ${rpg.emoticon(paymentMethod)}${capitalize(paymentMethod)}`
    }).join('\n')}
–––––––––––––––––––––––––
💁🏻‍♂ ᴛɪᴩ :
➠ ᴛᴏ ʙᴜʏ ɪᴛᴇᴍs:
...
`.trim())
    ...
    if (command.toLowerCase() == 'buy') {
        ...
        if (user[item] < total) return m.reply(`Kamu tidak punya cukup *${capitalize(item)} ${rpg.emoticon(item)}* untuk dijual, kamu hanya punya ${user[item]} item`)
        user[item] -= total
